...
namespace MonoMac.WebKit
{
	...
	interface WKHttpCookieStore
	{
		...
		[Export ("addObserver:")]
		void AddObserver (NSObject /*IWKHttpCookieStoreObserver*/ observer);
	
		[Export ("removeObserver:")]
		void RemoveObserver (NSObject /*IWKHttpCookieStoreObserver*/ observer);
	}
	...
	interface WKUrlSchemeHandler
	{
		...
		void StartUrlSchemeTask (WKWebView webView, /*IWKUrlSchemeTask*/ NSObject urlSchemeTask);
	
		[Abstract]
		[Export ("webView:stopURLSchemeTask:")]
		void StopUrlSchemeTask (WKWebView webView, /*IWKUrlSchemeTask*/ NSObject urlSchemeTask);
	}
	...
	interface WKUserContentController { // : NSSecureCoding {
		...
		void AddScriptMessageHandler (/*IWKScriptMessageHandler*/ NSObject scriptMessageHandler, WKContentWorld world, string name);

		[Mac (11,0), iOS (14,0)]
		[Export ("addScriptMessageHandlerWithReply:contentWorld:name:")]
		void AddScriptMessageHandlerWithReply (/*IWKScriptMessageHandlerWithReply*/ NSObject scriptMessageHandlerWithReply, WKContentWorld contentWorld, string name);
...
#endif
	{
		...
		void Find (string theString, [NullAllowed] WKFindConfiguration configuration, Action<WKFindResult> completionHandler);
	...
	interface WKWebViewConfiguration { // : NSCopying, NSSecureCoding {
		...
		void SetUrlSchemeHandler ([NullAllowed] /*IWKUrlSchemeHandler*/ NSObject urlSchemeHandler, string urlScheme);
	
		[Mac (10,13), iOS (11,0)]
		[Export ("urlSchemeHandlerForURLScheme:")]
		[return: NullAllowed]
		/*IWKUrlSchemeHandler*/ NSObject GetUrlSchemeHandler (string urlScheme);
